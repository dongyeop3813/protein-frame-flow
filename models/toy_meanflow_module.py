...
from data.interpolant import MeanFlowInterpolant
...
class MeanFlowModule(LightningModule):
    ...
    def __init__(self, cfg):
        ...
        # Set-up interpolant
        self.interpolant = MeanFlowInterpolant(cfg.interpolant)
    ...
    def model_step(self, noisy_batch: Any):
        ...
        # assert loss_mask.bool().all(), "Loss mask contains False values"
        ...
        v_trans = (trans_1 - trans_t) / norm_scale
        v_rot = so3_utils.calc_rot_vf(rot_t, rot_1.type(torch.float32)) / norm_scale
        if torch.any(torch.isnan(v_rot)):
            raise ValueError("NaN encountered in v_rot")

        # Model output predictions.
        model_output, d_model_output = self.model.jvp_avg_vel(
            noisy_batch,
            (
                v_trans,
                so3_utils.vector_to_skew_matrix(v_rot),
                torch.ones_like(t),
                torch.zeros_like(r),
            ),
        )

        # MeanFlow on translation.
        # u_trans and d_trans_u are tangent vectors at Euclidean space.
        u_trans = model_output["pred_trans_vf"]
        d_trans_u = d_model_output["pred_trans_vf"]
        u_trans_tgt = (v_trans - (r - t)[..., None] * d_trans_u).detach()

        trans_error = torch.sum(
            ((u_trans - u_trans_tgt) * loss_mask[..., None]) ** 2, dim=(-1, -2)
        )
        trans_loss = weighted_loss(trans_error, training_cfg.trans_loss_p)
        trans_loss *= training_cfg.translation_loss_weight / loss_denom
        trans_loss = torch.clamp(trans_loss, max=5)

        # MeanFlow on rotation.
        # u_rot and d_rot_u are tangent vectors (rotation vectors)at SO(3) manifold.
        # Shape of u_rot [B, N, 3], d_rot_u [B, N, 3]
        u_rot = model_output["pred_rot_vf"]
        d_rot_u = d_model_output["pred_rot_vf"]
        hat_rot_r = so3_utils.exp(rot_t, (r - t)[..., None] * u_rot)
        u_rot_tgt = (
            so3_utils.d1_log(rot_t, hat_rot_r, v_rot) - (r - t)[..., None] * d_rot_u
        ).detach()
        rot_error = torch.sum(
            ((u_rot - u_rot_tgt) * loss_mask[..., None]) ** 2, dim=(-1, -2)
        )
        rot_loss = weighted_loss(rot_error, training_cfg.rot_loss_p)
        rot_loss *= training_cfg.rotation_loss_weights / loss_denom
        rot_loss = torch.clamp(rot_loss, max=5)
    ...
    def predict_step(self, batch, batch_idx):
        ...
        interpolant = MeanFlowInterpolant(self._infer_cfg.interpolant)

...
from data.time_sampler import create_time_sampler
import copy
...
class MeanFlowInterpolant:
    ...
    def sample_t_and_r(self, num_batch):
        t, r = self.time_sampler(num_batch, self._device)
        return t, r
    ...
    def corrupt_batch(self, batch):
        ...
        # [B, 1]
        t, r = self.sample_t_and_r(num_batch)
        t, r = t[:, None], r[:, None]
        so3_t = t
        r3_t = t
        noisy_batch["so3_t"] = so3_t
    ...
    def forward_flow(
        ...
        # Set feats["res_mask"], feats["res_idx"], feats["diffuse_mask"]
        res_mask = torch.ones(num_batch, num_res, device=self._device)
        if res_idx is None:
            res_idx = torch.arange(num_res, device=self._device, dtype=torch.float32)[
                None
        ...
        r = torch.ones((num_batch, 1), device=self._device) * r
        ...
        with torch.no_grad():
            ...
            if self._sample_cfg.get("last_step", True):
                ...
                trans_r, rotmats_r = model(trans_r, rotmats_r, t, r, feats)
...
class SplitMeanFlowInterpolant(MeanFlowInterpolant):
    def sample_t_s_r(self, num_batch):
        t, s, r = self.time_sampler(num_batch, self._device)
        return t, s, r
    ...
    def corrupt_batch(self, batch):
        ...
        if self._cfg.flow_matching_use_new_t:
            noisy_batch["flow_matching_t"] = self.sample_t(num_batch)[:, None]
        else:
            noisy_batch["flow_matching_t"] = s
